# week09-3.py學習計畫 Linked List 第3題 Easy題 (使用「函式呼叫函式」)
# week09-2.py學習計畫 Linked List 第3題 Easy題
...
class Solution:
    def reverseList(self, head: Optional[ListNode]) -> Optional[ListNode]:
        if head == None or head.next == None: # 終止條件(最簡單狀況)
            return head
        head2 = head.next
        ...
        while head: # 只要還有資料
            ...
            head = head.next # 換下一筆
        # print(a) 印出來, 成功變成我們習慣的陣列
        now = ans = ListNode() # 答案將串到裡面

        #　下面用到過來的迴圈，把陣列的值，逐一加入
